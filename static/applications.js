...
async function loadApplications() {
    ...
    if (!response.ok) {
        trackerStatus.textContent = UI.formatApiError(response, payload, { includeStatus: true });
        body.innerHTML = '<tr><td colspan="6" class="empty-row">Could not load applications.</td></tr>';
        pagination.total = 0;
    ...
    if (applications.length === 0) {
        ...
        body.innerHTML = '<tr><td colspan="6" class="empty-row">No tracked applications yet.</td></tr>';
        return;
    }
...
function applicationRow(application) {
    ...
    for (const value of ["planned", "applied", "interviewing", "rejected", "offer", "archived"]) {
    ...
    followUpInput.addEventListener("change", () => {
        updateApplication(application.id, { follow_up_date: followUpInput.value || null });
    });
...
function updatePaginationControls({ rangeStart, rangeEnd }) {
    ...
    pageRange.textContent = pagination.total === 0 ? "0 of 0" : `${rangeStart}-${rangeEnd} of ${pagination.total}`;
