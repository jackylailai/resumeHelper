...
The screenshots in this guide are labeled directly on the image, so a first-time
reader can understand what to do without jumping back and forth to the tables.
The tables below each image provide the exact API and state-change details.

Traditional Chinese version: [ui-flow.zh-TW.md](ui-flow.zh-TW.md).
...
![Labeled Evaluate screen](assets/ui-flow/evaluate-result.labeled.png)
...
![Labeled Profile screen](assets/ui-flow/profile-add-form.labeled.png)
...
![Labeled JD Database screen](assets/ui-flow/jd-batch-result.labeled.png)
...
![Labeled History screen](assets/ui-flow/history.labeled.png)
...
![Labeled Submittable screen](assets/ui-flow/submittable.labeled.png)
...
![Labeled Applications screen](assets/ui-flow/applications-updated.labeled.png)
...
3. Regenerate the labeled images with
   `powershell -ExecutionPolicy Bypass -File scripts/annotate_ui_flow_screenshots.ps1`
   and update the callout tables if button behavior changed.
